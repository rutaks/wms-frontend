...
const ResetPassword = () => {
  ...
  useHandleApiState(resetPassword, {
    onSuccess: () => {
      ...
      <div style={{ height: '100vh' }}>
        <div style={{ paddingTop: '30vh' }}>
          <Result
            status="success"
            title="All set!"
            subTitle="You can now login into the system!"
            extra={[<Button onClick={() => history.replace('/login')}>Go to login</Button>]}
          />
        </div>
      </div>
    );
      ...
      <div style={{ paddingTop: '30vh', width: '100%' }}>
        ...
        <Row justify="center">
          <Formik
            ...
            onSubmit={(formData) => {
              resetPassword.sendRequest({ ...formData, token });
            }}
          >
            {(formikProps) => {
              return (
